...
一个作业（Job）由一组任务（Task）及其依赖关系组成，每个任务可以有一个或多个实例（Instance）。在提交作业之前，用户必须创建任务模板，任务模板包括：
...
在创建作业中指定，优先级范围：0-100，数值越大，优先级越高。

任务执行顺序：

作业中的多个任务之间可以有 DAG 依赖关系。即前面的任务运行完成后, 后面的任务才开始运行。DAG关系可在创建作业的任务模板画布中指定。
